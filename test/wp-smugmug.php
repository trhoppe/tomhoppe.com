		...
		function wp_smugmug_shortcode($atts, $content = null)
		{
			...
			if ( !empty($url) ) {
				...
				if ( is_wp_error($rss) || empty($rss) || count($rss->get_items()) == 0 ) {
					...
					$output .= '<ul class="thumbwrap" data-contentId="content-' . $post->ID . '">';

					$colorboxDiv = '<div style="display:none;"><div id="content-' . $post->ID . '" class="royalSlider rsDefault" style="width:100%; height:100%;">';
					...
					foreach($items as $item) {
						...
						if ( count($enclosures) > 1 ) {
							...
							$anchor_tag = '<a href="' . $the_link . '" title="' . $item_title . '">';

							$colorboxDiv .= '<a class="rsImg" href="' . $the_link . '" />';
